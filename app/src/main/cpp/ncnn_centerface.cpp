...
namespace Face{
    ...
    Centerface::Centerface(const std::string model_path) {
        const std::string param = model_path + "/centerface.param";
        const std::string bin = model_path + "/centerface.bin";
    ...
    void Centerface::SetThreadNum(int threadNum) {
        threadnum = threadNum;
    }

    int Centerface::detect(ncnn::Mat &inblob, std::vector <FaceInfo> &faces, int resized_w,
                           int resized_h, float scoreThresh, float nmsThresh) {
        if (inblob.empty()) {
        ...
        while (vScores.size() > 0) {
            ...
            for (std::multimap<float, int>::iterator it = vScores.begin(); it != vScores.end();) {
                ...
                if (type == NMS_UNION)
                    ...
                    IOU = IOU /
                          ((input.at(it_idx).area < input.at(last).area) ? input.at(it_idx).area
                                                                         : input.at(last).area);
                }
                if (IOU > nmsthreshold) {
                    it = vScores.erase(it);
    ...
    void Centerface::decode(ncnn::Mat &heatmap, ncnn::Mat &scale, ncnn::Mat &offset,
                            ncnn::Mat &landmarks, std::vector <FaceInfo> &faces, float scoreThresh,
                            float nmsThresh) {
        int fea_h = heatmap.h;
        int fea_w = heatmap.w;
